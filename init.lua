...
local render_numbers = ya.sync(function(_, mode, styles, resizable_entity_children_ids)
	...
	Current.render = function(current_self)
		...
		for i, f in ipairs(files) do
			...
			local linemode_rendered = Linemode:new(f):render()
			...
			if resizable_entity_children_ids and #resizable_entity_children_ids > 0 then
				...
				entity.render = function(entity_self)
					...
					entity.highlights = function(entity_highlight_self)
						...
						local extension = entity_highlight_self._file.url.ext(entity_highlight_self._file.url)
						...
						for _, h in ipairs(highlights) do
							...
							highlight_spans[#highlight_spans + 1] = ui.Span(shortened_name.result:sub(h[1] + 1, h[2]))
								:style(THEME.manager.find_keyword)
							last = h[2]
						end
				...
				entities[#entities + 1] = ui.Line(entity:render())
			else
				entities[#entities + 1] = ui.Line({ line_number_ui_component, entity:render() }):style(entity:style())
			end
